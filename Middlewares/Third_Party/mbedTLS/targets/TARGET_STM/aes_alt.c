...
static int aes_set_key(mbedtls_aes_context *ctx, const unsigned char *key, unsigned int keybits)
{
    ...
    ctx->hcryp_aes.Init.DataType = CRYP_DATATYPE_32B;
                          ...
                          unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        if (HAL_CRYP_Decrypt(&ctx->hcryp_aes, (uint32_t *)&input, (length/4), (uint32_t *)&output, ST_AES_TIMEOUT) != HAL_OK) {
            return ST_ERR_AES_BUSY;
        }
        /* Save the internal IV vector for multi context purpose */
        tickstart = HAL_GetTick();
        ...
        *iv_ptr++ = ctx->hcryp_aes.Instance->IV1RR;
        ...
        if (HAL_CRYP_Encrypt(&ctx->hcryp_aes, (uint32_t *)&input, (length/4), (uint32_t *)&output, ST_AES_TIMEOUT) != HAL_OK) {
            return ST_ERR_AES_BUSY;
        }
        memcpy(iv, output, 16);   /* current output is the IV vector for the next call */
        ctx->ctx_save_cr = ctx->hcryp_aes.Instance->CR;
                                 ...
                                 unsigned char output[16])
{
    ...
    if (HAL_CRYP_Encrypt(&ctx->hcryp_aes, temp, 4, temp, ST_AES_TIMEOUT) != HAL_OK) {
        // error found
        return ST_ERR_AES_BUSY;
    }
                                 ...
                                 unsigned char output[16])
{
    ...
    if (HAL_CRYP_Decrypt(&ctx->hcryp_aes, temp, 4, temp, ST_AES_TIMEOUT) != HAL_OK) {
        // error found
        return ST_ERR_AES_BUSY;
    }
    memcpy(&output, &temp, sizeof(temp));
