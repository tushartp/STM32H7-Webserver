...
lws_h2_rst_stream(struct lws *wsi, uint32_t err, const char *reason)
{
	...
	lwsl_info("%s: RST_STREAM 0x%lx, REASON '%s'\n", __func__, err, reason);
	...
	while (len >= LWS_H2_SETTINGS_LEN) {
		...
		case H2SET_INITIAL_WINDOW_SIZE:
			...
			lws_start_foreach_ll(struct lws *, w,
					     nwsi->h2.child_list) {
				lwsl_info("%s: adi child tc cr %d +%lu -> %lu",
					    __func__,
...
int lws_h2_do_pps_send(struct lws *wsi)
{
	...
	case LWS_H2_PPS_MY_SETTINGS:
		...
		for (n = 1; n < H2SET_COUNT; n++)
			if (h2n->set.s[n] != lws_h2_defaults.s[n]) {
				lwsl_debug("sending SETTING %d 0x%lx\n", n,
						wsi->h2.h2n->set.s[n]);
	...
	case LWS_H2_PPS_UPDATE_WINDOW:
		lwsl_debug("Issuing LWS_H2_PPS_UPDATE_WINDOW: sid %lu: add %lu\n",
...
lws_h2_parse_frame_header(struct lws *wsi)
{
	...
	lwsl_debug("%p (%p): fr hdr: typ 0x%x, fla 0x%x, sid 0x%lx, len 0x%lx\n",
	...
	if (h2n->length > h2n->set.s[H2SET_MAX_FRAME_SIZE]) {
		 ...
		 * it we would allow
		 */
		lwsl_info("received oversize frame %lu\n", h2n->length);
	...
	if (h2n->swsi)
		...
		if (h2n->sid > h2n->highest_sid_opened &&
			...
			lwsl_info("%s: wsi %p, No child for sid %lu, rxcmd %u\n",
	...
	if (h2n->cont_exp && (h2n->cont_exp_sid != h2n->sid ||
		...
		lwsl_info("%s: expected cont on sid %lu (got %u on sid %lu)\n",
	...
	case LWS_H2_FRAME_TYPE_DATA:
		...
		lwsl_info("Frame header DATA: sid %lu\n", h2n->sid);
	...
	case LWS_H2_FRAME_TYPE_CONTINUATION:
		lwsl_info("LWS_H2_FRAME_TYPE_CONTINUATION: sid = %lu\n",
			  h2n->sid);
	...
	case LWS_H2_FRAME_TYPE_HEADERS:
		lwsl_info("HEADERS: frame header: sid = %lu\n", h2n->sid);
		if (!h2n->sid) {
...
lws_h2_parse_end_of_frame(struct lws *wsi)
{
	...
	case LWS_H2_FRAME_TYPE_HEADERS:
		...
		if (h2n->hpack != HPKS_TYPE) {
			/* hpack incomplete */
			lwsl_info("hpack incomplete %d (type %u, len %lu)\n",
		...
		lwsl_debug("%s: setting DEF_ACT from 0x%lx\n", __func__,
			   h2n->swsi->wsistate);
	...
	case LWS_H2_FRAME_TYPE_WINDOW_UPDATE:
		...
		lwsl_info("WINDOW_UPDATE: sid %lu %lu (0x%lx)\n", h2n->sid,
	...
	case LWS_H2_FRAME_TYPE_GOAWAY:
		lwsl_info("GOAWAY: last sid %lu, error 0x%lx, string '%s'\n",
	...
	case LWS_H2_FRAME_TYPE_RST_STREAM:
		lwsl_info("LWS_H2_FRAME_TYPE_RST_STREAM: sid %lu: reason 0x%lx\n",
	...
	while (inlen--) {
		...
		case LRS_H2_AWAIT_SETTINGS:
			...
			if (h2n->flags & LWS_H2_FLAG_PRIORITY &&
				...
				lwsl_debug("PRI FL: dep 0x%lx, weight 0x%02X\n",
			...
			case LWS_H2_FRAME_TYPE_DATA:
...
#endif
				{
					...
					if (n < 0 ||
						...
						lwsl_info("%s: lws_read_h1 told %d %lu / %lu\n",
	...
	//				   h2n->swsi->h2.peer_tx_cr_est, wsi->h2.peer_tx_cr_est);
			...
			case LWS_H2_FRAME_TYPE_PRIORITY:
				if (h2n->count <= 4) {
					...
					h2n->weight_temp = c;
					lwsl_info("PRIORITY: dep 0x%lx, weight 0x%02X\n",
						  h2n->dep, h2n->weight_temp);
...
frame_end:
			if (h2n->count > h2n->length) {
				lwsl_notice("%s: count > length %lu %lu\n",
